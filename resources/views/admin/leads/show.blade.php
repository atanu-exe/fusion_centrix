                    ...
                    <div class="d-grid gap-2">
                        <form action="{{ route('admin.leads.convert', $lead) }}" method="POST" class="d-inline">
                            ...
                            <button type="submit" class="btn btn-success"
                                ...
                                <i class="fas fa-arrow-right-arrow-left me-1"></i>Convert to Client
                            </button>
                        </form>
        ...
        <div class="col-lg-8">
            ...
            <div class="card">
                ...
                <div class="card-body">
                            ...
                            @foreach ($lead->followups->sortByDesc('created_at') as $followup)
                                <div class="timeline-item pb-4 border-start ps-4 position-relative">
                                    ...
                                    <div class="d-flex justify-content-between align-items-start mb-1">
                                        <div>
                                            <span
                                                class="badge bg-{{ $followup->type === 'call' ? 'success' : ($followup->type === 'email' ? 'info' : 'secondary') }} me-2">
                                                <i
                                                    class="fas fa-{{ $followup->type === 'call' ? 'phone' : ($followup->type === 'email' ? 'envelope' : ($followup->type === 'meeting' ? 'users' : 'comment')) }} me-1"></i>
                                    ...
                                    @if (!$followup->is_completed)
                                        <form action="{{ route('admin.leads.followup.complete', $followup) }}"
                    ...
                    @else
                        <p class="text-muted text-center py-4 mb-0">No activity recorded yet. Add a follow-up to get
                            started.</p>
                    @endif
                </div>
            </div>
