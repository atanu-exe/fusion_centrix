                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        ...
                        <div class="mb-4 p-3 rounded" style="background-color: #f8f9fa; border-left: 4px solid #5f63f1;">
                            <label class="form-label mb-2" style="font-weight: 600;"><i
                                    class="fas fa-file-alt me-2"></i>Start With Template</label>
                            <select name="template_id" class="form-select" id="templateSelect"
                                style="border-color: #5f63f1;">
                                <option value="">✏️ Start from scratch</option>
                                @foreach ($templates as $template)
                                    <option value="{{ $template->id }}" data-template-body="{!! htmlspecialchars($template->body) !!}"
                            ...
                            <small class="text-muted d-block mt-2">Choose a template to auto-fill the email content, or
                                start from scratch</small>
                        </div>
                        ...
                        <div class="mb-3">
                            <label class="form-label">Email Subject <span class="text-danger"
                                    id="subjectRequiredMark">*</span></label>
                            <input type="text" name="subject" class="form-control @error('subject') is-invalid @enderror"
                                value="{{ old('subject') }}" id="emailSubject" required
                        ...
                        <div class="mb-3" id="emailContentContainer">
                            ...
                            <textarea name="content" class="form-control @error('content') is-invalid @enderror" rows="15" id="emailContent"
                                required>{{ old('content') }}</textarea>
                            @error('content')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">HTML supported. Variables: {name}, {email}, {company},
                                {unsubscribe_link}</small>
                        </div>
            ...
            <div class="col-lg-4">
                        ...
                        <div class="mb-3" id="leadSelect" style="display: none;">
                            ...
                            <select name="lead_ids[]" class="form-select" multiple size="10">
                                @foreach ($leads as $lead)
                                    <option value="{{ $lead->id }}">{{ $lead->name }} ({{ $lead->email }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="from_name" class="form-control"
                                value="{{ old('from_name', $settings_email_form_name ?? config('mail.from.name')) }}">
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Reply To</label>
                            <input type="email" name="reply_to" class="form-control"
                                value="{{ old('reply_to', $settings_email_reply_to ?? config('mail.from.address')) }}">
                        </div>
                    </div>
                </div>
                ...
                <div class="d-grid gap-2">
                    ...
                    <button type="submit" name="action" value="send" class="btn btn-success"
                        onclick="return confirm('Send this campaign immediately?')">
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" name="action" value="schedule" class="btn btn-info">Schedule
                            Campaign</button>
                    </div>
                </div>
            </div>
